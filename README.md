...
A collection of practical security tools built while completing [**AI Cyber Defense Ops**](https://www.justhacking.com/course/ai-cyber-defense-ops/) - a hands-on course on using Claude to build real blue-team tooling, by Just Hacking Training, with instructor Anton Ovrutsky.
...
|---|---|---|---|
| [`phishing-triage`](phishing-triage/) | **Course capstone project.** A static, offline **Phishing Triage Copilot** for a domain the course never covered (email), composing five Claude Code mechanisms into one tool: an **MCP server** (`analyze_email` / `defang_iocs` / `generate_report`) that parses an `.eml`, checks SPF/DKIM/DMARC and sender-spoofing, extracts and **defangs** URL/IP/attachment IOCs, scores it, and maps to MITRE ATT&CK — plus a `phishing://indicators` **resource**, a `phishing-triage` **skill**, a `/triage-email` **slash command**, a `soc-analyst` **persona**, and a `PreToolUse` **hook** that blocks any URL fetch so a phishing link can never be detonated. Produces a self-contained interactive **HTML triage report** (the repo's first Module 9 report/artifact). Analysis is 100% static — no network, nothing detonated; the email is treated as untrusted, prompt-injection-carrying data throughout | Capstone Project (new domain; composes Modules 3–7, 9, 11) | Python (stdlib: `email`, `hashlib`; only dep is `mcp`), MCP SDK, Bash + jq hook, MITRE ATT&CK |
| [`personas`](personas/) | Three composable system prompts that constrain Claude Code into a specific security-analyst role - **threat-hunter** (hypothesis-driven hunting), **ir-responder** (evidence-preserving incident response), **detection-engineer** (skeptical Sigma rule author) - each with its own hard behavioral constraints (no host-modifying suggestions, no hand-waved FP analysis, no treating a hypothesis as confirmed without evidence) and an enforced output contract. Loaded via `--append-system-prompt-file`, with worked example sessions per persona; the detection-engineer example is grounded in the real Matanbuchus/ClickFix threat intel from the `purple-team` exercise rather than a generic pattern | Module 11 - System Prompts for Security Personas | Claude Code system prompts (`--append-system-prompt-file`), MITRE ATT&CK, Sigma, KQL/SPL |
| [`purple-team`](purple-team/) | The end-to-end workflow that stitches the earlier pieces into one **purple-team loop**: threat intel → Atomic Red Team mapping → attacker simulation → detection scan (Hayabusa MCP) → SIEM validation → gap analysis → sanitized report. Adds the piece needed to actually close the loop - a small **synthetic-EVTX writer** that turns rendered event XML into a valid binary `.evtx` the detection engine will parse (debugged past a *zero-events* parse failure caused by chunk-absolute BinXML name offsets). Ships a worked exercise validating T1069.002 Domain Groups and T1136.001/T1098 Create Local Admin - 21 rule hits over 10 events, both techniques caught at high severity, with one real gap identified (a localized group-name add that generates no audit event). Report deliverable is programmatically stripped of credentials, keys, hashes, and account IDs | Module 10 - End-to-End Purple Team Workflow | Claude Code slash commands + subagent, Hayabusa MCP, MITRE ATT&CK, Atomic Red Team, Python (binary EVTX encoder, PDF) |
| [`complex-analysis`](complex-analysis/) | Multi-step security analysis built on Claude Code **subagents**: purpose-scoped `endpoint-analyst` (Windows Security + Sysmon) and `cloud-analyst` (Azure AD sign-in + audit) run in parallel over their own log domains, then a correlation step joins their notes into one incident picture - timeline alignment, user + IP correlation, unified attack chain, confidence, gaps, containment. Also ships a `/ingest-ti` slash command that turns a threat-intel report into MITRE ATT&CK TTPs + an Atomic Red Team simulation plan. Fan-out/fan-in over file artifacts (not chat state), so each stage is rerunnable and hand-off-friendly | Module 8 - Complex Analysis: Threat Intel & Multi-Source Correlation | Claude Code subagents (`.claude/agents/`), slash commands, JSON log parsing, MITRE ATT&CK, Atomic Red Team |
...
*(`phishing-triage` is the course capstone — a new-domain project that ties several of the mechanisms above into one tool. The others are the per-module builds.)*
...
- Decomposing an investigation across purpose-scoped **subagents** (one log domain each, least-privilege toolset, fixed output contract) with a fan-out/fan-in pattern - parallel per-domain analysis merged by a correlation step that reads file artifacts, not chat state, so every stage is rerunnable and hand-off-friendly; plus handling a subagent aborting on an automated cyber-safeguard false positive by completing the (legitimate defensive) analysis inline
...
ai-cyber-defense-tools/
├── README.md                 <- this file
├── phishing-triage/          <- capstone: MCP server + skill + command + hook + persona + HTML report
│   ├── README.md
│   ├── server.py             <- MCP tools; analyze.py/report.py hold the logic
│   ├── samples/              <- synthetic .eml files (1 benign, 2 phish)
│   └── ...
├── detection-workflow/
│   ├── README.md
│   └── ...
...
├── complex-analysis/
│   ├── README.md
│   ├── .claude/agents/       <- endpoint-analyst, cloud-analyst subagents
│   ├── logs/{windows,cloud}/ <- sample endpoint + cloud telemetry
│   └── analysis/             <- per-domain notes + correlation
└── ...
```
